...
app.use('/javascript/library/', express.static(__dirname + "/javascript/library/"));
	...
	fs.readFile('maked_branch2.html', 'utf8', function (error, data) {
		if (!error) {
			GetMemberDataFromYear('2016', 2, function (AB) {
				...
				// newBSList.forEach(function(element) {
				// 	console.log('bs:' + element.RowKey._);
				// }, this);
...
app.get('/test', function (request, response) {
	fs.readFile('csTest.html', 'utf8', function (error, data) {
		response.send(data);
	});
});
...
app.post('/branch_edit/:id', function (request, response) {
	...
	// 	blobService.createBlockBlobFromFile('imgcontainer', 'test', file.path, options, function(error) {
	// 		if (!error) {
    ...
    form.on('field', function (field, value) {
		...
		// var thumbURL = fields['thumbURL'];
		
		// if (field == "thumbURL" && value != null && value != "") {
		...
		// 	};
		// 	var container = 'imgcontainer';
		// 	var size = imageStream.byteCount;
			
		// 	blobService.createBlockBlobFromStream(container,'testThumb.jpg', imageStream, size, options, function(error, blobResult, response) {
		// 		if (error)
		...
		// 	});
			...
			// var url_decoded = b64decode(thumbURL.encode())
			// base64.base64encoder(value, "thumb", function(err, saved) {
			// 	if (!err) {
			// 		console.log(save);
...
app.post('/branch_profile', function (request, response) {
	...
	// 파일을 읽습니다.
	fs.readFile('profile_template2.ejs', 'utf8', function (error, data) {
		var query = new azure.TableQuery()
		.top(1)
		.where('RowKey eq ?', id);
		...
		tableService.queryEntities('members', query, null, function entitiesQueried(error, result) {
			if (!error) {
				...
				var getData = {};
				getData.birthYear = (entries[0].PartitionKey != null) ? entries[0].PartitionKey._ : null;
				getData.birthMonth = (entries[0].birthMonth != null) ? entries[0].birthMonth._ : null;
				getData.birthDay = (entries[0].birthDay != null) ? entries[0].birthDay._ : null;
				getData.branch = request.body.branch;
				getData.name = (entries[0].RowKey != null) ? entries[0].RowKey._ : null;
				getData.attend = request.body.attend;
				...
				response.send({
					result: true,
					message: ejs.render(data,getData)
				});
...
/* 이 아래는 전부 테스트 용이기 때문에 쓰이는 일은 없음 */
